...
class HISANIM_PT_LIGHTDIST(bpy.types.Panel):
    ...
    def draw(self, context):
        ...
        if context.scene.hisaenablelightdist and context.scene.camera == None:
            row = layout.row()
            row.label(text='No Camera Active!')
        row = layout.row()
        row.prop(context.scene.hisanimdrag, 'value', text='Distance Threshold')
        row.enabled = True if context.scene.hisaenablelightdist and context.scene.camera != None else False
        layout.row().label(text='Overrides')
        row = layout.row(align=False)
        row.operator('hisanim.lightoverride', text="Always On")
        row.operator('hisanim.hidelight', text='Always Off')
        row.operator('hisanim.removeoverrides', text='Remove Override')
        ...
        row = layout.row()
        row.prop(context.scene, 'hisanimmodulo', text='Optimize After # of Frames')
        row = layout.row()
        ...
        layout.row().prop(context.scene, 'prx_frustrum')
        row = layout.row()
        ...
        row=layout.row()
        row.prop(context.scene, 'hisanimlightstats', text='Light Statistics (Disabling may improve performance)')
...
def ExcludeLight(a, b, c= None): # multi-purpose light excluder.
    ...
    if b == 'SHOW' and a.data.get('DEFAULT') != None:
        LIGHT = a
            ...
            #LIGHT.data.energy  = MAP(math.dist(camloc, LIGHT.matrix_world.translation), DIST-(CS.hisanimstartfrom*DIST), DIST-(CS.hisanimendat*DIST), 0, a.get('POWER'), True)
...
def is_origin_in_view(obj):
    # Get the camera frustum
    if not bpy.context.scene.prx_frustrum: return True
    camera = bpy.context.scene.camera
    camera_location = camera.matrix_world.translation

    # Get the object's origin point in world space
    origin = obj.matrix_world.translation

    # Calculate the direction vector from the camera to the object
    direction = origin - camera_location

    # Check if the object's origin is behind the camera
    if direction.dot(camera.matrix_world.to_quaternion() @ Vector((0, 0, -1))) <= 0:
        return False
...
class HISANIM_OT_DRAGSUBSCRIBE(bpy.types.Operator):
    ...
    def modal(self, context, event):
        if self.stop:
            ...
            #bpy.data.meshes.remove(bpy.context.active_object.data)
            bpy.data.objects.remove(bpy.context.active_object)
            context.view_layer.objects.active = context.scene.prxlightlast
            
            if context.scene.prxlightlast != None: context.scene.prxlightlast.select_set(True)
            return {'FINISHED'}
...
def hisanimupdates(self, value):
    ...
    # taken from https://blender.stackexchange.com/questions/245233/drag-events-of-a-panel-ui-slider
    C = bpy.context
    camloc = C.scene.camera.matrix_world.translation
    if self.is_dragging:
        OptimizeLights()
        # scale the empty while dragging
        bpy.data.objects['LIGHTDIST'].scale = Vector((self.value, self.value, self.value))
    else:
        bpy.context.scene.prxlightlast = bpy.context.active_object
        # create an empty shaped as a sphere, scaled to the distanced from the camera defined by the user
        bpy.ops.mesh.primitive_ico_sphere_add(subdivisions=4)
        
        if (mat := bpy.data.materials.get('PRXLIGHTMAT')) == None:
            
            mat = bpy.data.materials.new('PRXLIGHTMAT')

            mat.use_nodes = True

            mat.node_tree.nodes.remove(mat.node_tree.nodes['Principled BSDF'])

            node1 = mat.node_tree.nodes.new('ShaderNodeNewGeometry')
            
            node2 = mat.node_tree.nodes.new('ShaderNodeMixShader')
            
            node3 = mat.node_tree.nodes.new('ShaderNodeBsdfTransparent')
            
            node4 = mat.node_tree.nodes.new('ShaderNodeBsdfDiffuse')

            mat.node_tree.links.new(node1.outputs['Backfacing'], node2.inputs['Fac'])

            mat.node_tree.links.new(node3.outputs[0], node2.inputs[1])
            
            mat.node_tree.links.new(node4.outputs[0], node2.inputs[2])

            out = mat.node_tree.nodes['Material Output']

            mat.node_tree.links.new(node2.outputs[0], out.inputs['Surface'])
...
class HISANIM_LIGHTDIST(bpy.types.PropertyGroup):
    value: bpy.props.FloatProperty(update=hisanimupdates, min=0.0, max=1000.0, step=25, default=25.0, description='If the distance between a light and camera exceeds this threshold, it will get disabled')
...
def OptimizeLights(self = None, context = None):
    CS = bpy.context.scene
    camloc = CS.camera.matrix_world.translation
    LIGHTDICT = {}
    ...
    if CS.hisanimhardclamp: # if light clamping is enabled
        ...
        for i in LIGHTS:
            if i.data.type == 'SUN' or i.get('LIGHTOVERRIDE') != None:
                ...
                continue
            LIGHTDICT[i.name] = math.dist(camloc, i.matrix_world.translation)
    ...
    if CS.hisanimenablespread and bpy.context.screen.is_animation_playing and CS.hisanimmodulo:
        ...
        for i in l[int((f() % t())*(len(l) / t())) : None if int(((f() + 1) % t())*(len(l) / t()))== 0 else int(((f() + 1) % t())*(len(l) / t()))]: # good luck figuring that out lol
            ...
            if LIGHTS[i].get('LIGHTOVERRIDE') or LIGHTS[i].get('PERMHIDDEN') or LIGHTS[i].data.type != 'SUN': # if the light has an override property, skip it
                continue
            if (math.dist(camloc, LIGHTS[i].matrix_world.translation) > CS.hisanimdrag.value) or not is_origin_in_view(i): # if the light is outside of the defined distance and is not a sun, hide it 
                ExcludeLight(LIGHTS[i], 'HIDE')
            else: # do the opposite
                ExcludeLight(LIGHTS[i], 'SHOW')
                LIGHT = LIGHTS[i]
                if LIGHT.data.get('POWER') != None:
                    LIGHT.data.energy = MAP(math.dist(camloc, LIGHT.matrix_world.translation), DIST-(CS.hisanimstartfrom*DIST), DIST-(CS.hisanimendat*DIST), 0, LIGHT.data.get('POWER'), True)
        ...
        for i in LIGHTS:
            if i.get('LIGHTOVERRIDE') or i.get('PERMHIDDEN') or i.data.type == 'SUN':
                continue
            if math.dist(camloc, i.matrix_world.translation) > CS.hisanimdrag.value or not is_origin_in_view(i):# and not i.data.type == 'SUN':
                ExcludeLight(i, 'HIDE')
            else:
                ExcludeLight(i, 'SHOW')
                DIST = CS.hisanimdrag.value
                if i.data.get('POWER') != None:
                    i.data.energy = MAP(math.dist(camloc, i.matrix_world.translation), DIST-(CS.hisanimstartfrom*DIST), DIST-(CS.hisanimendat*DIST), 0, i.data.get('POWER'), True)
...
def register():
    ...
    bpy.types.Scene.hisanimdrag = bpy.props.PointerProperty(type=HISANIM_LIGHTDIST, description='Cursor will bug out, that is normal.')
    for func in bpy.app.handlers.frame_change_post:
        if func.__name__ == 'OptimizeLights':
    ...
    bpy.types.Scene.hisanimhardclamp = bpy.props.BoolProperty(name='Light Culling', description='Only a maximum amount of lights can be enabled')
    ...
    bpy.types.Scene.prx_frustrum = bpy.props.BoolProperty(name='Camera Frustrum', description='Lights will be disabled if out of view', default=False)
    bpy.types.Scene.prxlightlast = bpy.props.PointerProperty(type=bpy.types.Object)
    #bpy.types.Scene.hisanimpower = bpy.props.FloatProperty(name='Power', default=1.0, description='Applies an exponent when fading the lights')
def unregister(a, b):
    for i in classes:
...
bpy.app.handlers.load_pre.append(unregister)
